r"""Build an Episode Language Realization Plan from its three sources.
...
All inputs must be **canonical** bytes, exactly what their writers emitted,
...
The command is a thin shell around ``living_diorama.language_realization``: it
words no sentence of its own, and every refusal comes from the contract rather
than from here. Before anything is written, the freshly built plan is
cross-validated against all three inputs, so a realization plan file can never
exist without its bindings having been proven against the actual sources at
least once.

There is no delivery plan, no shot plan, and no render manifest input. A
realized sentence is presentation wording, settled once the story is narrated;
joining sentences to windows, voices and frames is the later layers' work.
"""
...
def _read_canonical(path: Path, description: str) -> object:
    ...
    if raw != dumps_canonical(document, description):
        raise ValueError(
            f"{description} at {path} is not canonical bytes. The realization plan binds "
            "the digest of every document it read, so each file must be exactly what its "
...
def build(narration_path: Path, story_path: Path, export_path: Path, output_path: Path) -> int:
    ...
    # strict reader as every other document this command touches, rather than a
    # plain json.loads of bytes this process itself just emitted.
    validate_language_realization_plan_against_sources(
        loads_canonical(payload, "language realization plan"), narration, story, export
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    """Parse arguments, write the plan, and report what was realized."""
    parser = argparse.ArgumentParser(
        prog="build_language_realization_plan",
        description="Derive an Episode Language Realization Plan from its three sources.",
    )
    parser.add_argument("--narration", required=True, help="the Episode Narration Plan to realize")
    parser.add_argument(
        "--story", required=True, help="the Episode Story Plan whose evidence licenses it"
    )
    parser.add_argument(
    ...
    document = cast(
        dict[str, Any],
        loads_canonical(Path(namespace.output).read_bytes(), "language realization plan"),
    )
    counts = {
        "bytes": written,
        "episode": document["source"]["episode"],
        "fact_backed": document["accounting"]["fact_backed"],
        "mode": document["source"]["mode"],
        "realizations": document["accounting"]["realizations_total"],
        "template_backed": document["accounting"]["template_backed"],
    }
