"""In-Blender structural test runner for Phase 18 (superset of Phase 17).
...
Phase 17 motion suite, and only then the Phase 18 population suite. Presence
can therefore never be made to pass by weakening anything that came before it.

Usage::

    blender --background --factory-startup --python run_blender_tests_p18.py \
...
def main() -> int:
    ...
    print(f"LD_BLENDER_TESTS_P18: {executed - failures} passed, {failures} failed")
