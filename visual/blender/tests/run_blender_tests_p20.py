"""In-Blender structural test runner for Phase 20 (superset of Phase 19).
...
LOCKED Phase 19 mobility suite, and only then the Phase 20 state-response
suite. The world's visible condition can therefore never be made to pass by
weakening anything that came before it.

Usage::

    blender --background --factory-startup --python run_blender_tests_p20.py \
...
def main() -> int:
    ...
    print(f"LD_BLENDER_TESTS_P20: {executed - failures} passed, {failures} failed")
