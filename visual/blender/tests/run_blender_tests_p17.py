"""In-Blender structural test runner for Phase 17 (superset of Phase 16).
...
LOCKED Phase 16 production suite over the city it builds, and only then the
Phase 17 motion suite. Motion can therefore never be made to pass by
weakening anything that came before it.

Usage::

    blender --background --factory-startup --python run_blender_tests_p17.py \
        -- --spec master_scene_v1.json --production production_world_v1.json \
        --motion motion_time_v1.json --before before.json --mid mid.json \
        --after after.json --workdir <scratch dir>
"""
...
def main() -> int:
    ...
    print(f"LD_BLENDER_TESTS_P17: {executed - failures} passed, {failures} failed")
