"""The CLI: strict canonical reads over seven input documents, and clean refusals.
...
must leave nothing behind. There are exactly seven canonical INPUT
documents; the output path is a destination, never read through the strict
canonical decoder.
"""
...
CLI_SOURCE = REPO_ROOT / "src" / "living_diorama" / "cli" / "build_voice_plan.py"
...
def _write_inputs(tmp_path: Path) -> list[Path]:
    ...
    paths[0].write_bytes(dumps_canonical(realization, "realization"))
    paths[1].write_bytes(dumps_canonical(presentation, "presentation"))
    paths[2].write_bytes(dumps_canonical(delivery, "delivery"))
    paths[3].write_bytes(dumps_canonical(narration, "narration"))
    paths[4].write_bytes(dumps_canonical(shots, "shots"))
    paths[5].write_bytes(dumps_canonical(story, "story"))
    paths[6].write_bytes(dumps_canonical(export, "export"))
    return paths
...
def test_the_command_writes_a_verified_plan(tmp_path: Path, capsys: pytest.CaptureFixture) -> None:
    """The happy path writes canonical bytes and reports structural counts."""
    paths = _write_inputs(tmp_path)
    assert main(_argv(paths)) == 0
    payload = paths[7].read_bytes()
    document = loads_canonical(payload, "plan")
    assert payload == dumps_canonical(document, "plan")
    counts = json.loads(capsys.readouterr().out)
    assert counts["voice_units_total"] == 3
    assert counts["capacity_samples_total"] == 648_000
...
def test_an_existing_output_is_never_overwritten(
    ...
    paths = _write_inputs(tmp_path)
    paths[7].write_bytes(b"occupied\n")
    assert main(_argv(paths)) == 1
    err = capsys.readouterr().err
    assert "already exists" in err
    assert "Traceback" not in err
    assert paths[7].read_bytes() == b"occupied\n"
...
def test_pretty_printed_input_is_refused(
    ...
    """Pretty printed input is refused -- for each of the seven canonical inputs."""
    paths = _write_inputs(tmp_path)
...
def test_a_duplicate_json_key_is_refused(
    ...
    """A duplicate json key is refused, for each of the seven canonical inputs."""
    paths = _write_inputs(tmp_path)
...
def test_a_join_refusal_leaves_no_output(tmp_path: Path, capsys: pytest.CaptureFixture) -> None:
    ...
    paths = _write_inputs(tmp_path)
    _realization2, presentation2, *_ = build_voice_sources(2)
    paths[1].write_bytes(dumps_canonical(presentation2, "presentation"))
    assert main(_argv(paths)) == 1
    err = capsys.readouterr().err
    assert "Traceback" not in err
    assert not paths[7].exists()
...
def test_the_reused_upstream_gate_runs_before_write(
    tmp_path: Path, capsys: pytest.CaptureFixture
) -> None:
    """A forged shot plan is refused by the reused Phase 27 gate, not silently ignored."""
    paths = _write_inputs(tmp_path)
    _realization2, _presentation2, _delivery2, _narration2, shots2, *_ = build_voice_sources(2)
    paths[4].write_bytes(dumps_canonical(shots2, "shots"))
    assert main(_argv(paths)) == 1
    err = capsys.readouterr().err
    assert "Traceback" not in err
    assert not paths[7].exists()
...
def test_a_forged_story_plan_is_refused_by_the_reused_gate(
    tmp_path: Path, capsys: pytest.CaptureFixture
) -> None:
    """A forged story plan is refused by the reused gate."""
    paths = _write_inputs(tmp_path)
    _realization2, _presentation2, _delivery2, _narration2, _shots2, story2, _export2 = (
        build_voice_sources(2)
    )
    paths[5].write_bytes(dumps_canonical(story2, "story"))
    assert main(_argv(paths)) == 1
    err = capsys.readouterr().err
    assert "Traceback" not in err
    assert not paths[7].exists()
